---
name: media-renamer
description: Safely rename local video and subtitle files with the bundled MediaRenamer script. Use when a user asks to clean, standardize, preview, apply, undo, or report missing subtitles for media files and folders.
---
...
3. Run `--apply` only when explicitly authorized. It renames only safe files, deletes image/NFO sidecars from directories that changed, and creates `rename_log_*.json` plus `rename_undo_*.json` in the selected root.
...
Options: `--no-recursive`, `--movies`, `--series`, `--include-extras`, `--verbose`, `--config <json>`.
