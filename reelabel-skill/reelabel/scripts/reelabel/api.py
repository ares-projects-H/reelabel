...
def undo(
    ...
    *,
    ...
    restored, errors = core.undo(
        history_entry,
        scope=Path(scope_value),
        # The desktop app stores records outside media folders, but only in its
        # private history directory. Explicit-scope CLI records must instead
        # remain inside the renamed media root before Undo can authorize that
        # root-folder move.
        allow_external_root_move=trusted_history_dir is not None,
    )
    if not errors:
