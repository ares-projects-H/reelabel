...
4. Use `python scripts/rename_media.py --undo "<undo-json>" --undo-scope "<original-media-folder>"` to undo an apply operation without overwriting existing files or trusting paths outside the explicitly authorized folder.
...
python scripts/rename_media.py --undo "<undo-json>" --undo-scope "<original-media-folder>"
```

Options: `--no-recursive`, `--movies`, `--series`, `--include-extras`, `--delete-sidecars`, `--verbose`, `--config <json>`, `--undo-scope <folder>`.

## Safety

- Never run `--apply` from a vague request.
- Never add `--delete-sidecars` unless the user explicitly requests deletion.
- Only rename folders that appear in the preview and receive explicit approval.
- Never change extensions, media contents, or unrelated files.
